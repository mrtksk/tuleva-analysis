...
Minu võrldusindeks koosneb kolmest iShares ETF-ist: "ACWI" jäljendab maailma aktsiaturu indeksit MSCI ACWI, "IGOV" jäljendab maailma valitsuste võlakirju v.a. USA omi ja "AGG" jäljendab kogu USA investeerimisjärgu võlakirjaturgu. Need kaks võlakirja ETF-i katavad ära ligi 80% investeerimisjärgu võlakirjadest maailmas.
...
Eesti andmeid Tidyquanti'i abil kätte ei saa, kasutame otse csv faili allalaadimist. 

```{r echo=TRUE, message=FALSE, warning=FALSE}
#kuigi tahtsin püsida Tidyverse'i maailmas, toimib read.csv EVK andmete puhul paremini
...
* Swedbank K3-s `r format(SWK50, digits = 2)` protsenti  
* Nordea A-ss`r format(NPK50, digits =2)` protsenti  
* LHV L-is`r format(LLK50, digits =2)` protsenti  
* SEB Progressiivses `r format(SEK50, digits =2)` protsenti
...
* LHV L-s, oleks selle väärtus kasvanud `r a` miljardi euroni  
